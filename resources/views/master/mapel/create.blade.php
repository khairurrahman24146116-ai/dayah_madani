...
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="px-4 sm:px-6 py-4 bg-emerald-600">
        <h1 class="text-lg sm:text-xl font-bold text-white">Tambah Mata Pelajaran</h1>
    </div>
        ...
        <form action="{{ route('admin.mapel.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="kategori" id="kategori" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
                    ...
                    <option value="salafi" {{ old('kategori') == 'salafi' ? 'selected' : '' }}>Salafi</option>
                    <option value="umum" {{ old('kategori') == 'umum' ? 'selected' : '' }}>Umum</option>
                    <option value="pondok" {{ old('kategori') == 'pondok' ? 'selected' : '' }}>Pondok</option>
                </select>
                @error('kategori')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mobile-stack">
                ...
                <a href="{{ route('admin.mapel.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg text-sm font-medium transition text-center w-full sm:w-auto">Kembali</a>
            </div>
        </form>
    </div>
</div>
