...
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="px-4 sm:px-6 py-4 bg-emerald-600">
        <h1 class="text-lg sm:text-xl font-bold text-white">Manajemen KRS</h1>
    </div>
    <div class="p-4 sm:p-6">
        <form method="GET" class="filter-wrap mb-4">
            <label class="text-sm font-medium text-gray-700">Filter Status:</label>
            <select name="status" onchange="this.form.submit()" class="border border-gray-300 rounded-lg p-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 w-full sm:w-auto">
            ...
            <table class="w-full table-auto border-collapse table-card">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-sm text-gray-700" data-label="No">{{ $krsList->firstItem() + $i }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700" data-label="Nama Santri">{{ $krs->santri->nama_lengkap ?? '-' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700" data-label="NIS">{{ $krs->santri->nis ?? '-' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700" data-label="Kelas">{{ $krs->kelas->nama ?? '-' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700" data-label="Tahun Ajaran">{{ $krs->tahunAjaran->nama ?? $krs->tahun_ajaran ?? '-' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-6 text-center text-gray-500 empty-card">Belum ada data KRS.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
